...
func (e *Evpp) pingPong(conn *net.TCPConn) {
	...
	if err == nil {
		...
		if err == nil {
			//fmt.Println("conn Read len", length, string(recBuf))
...
func main() {
	...
	var N = flag.Int("N", 2, "the total number of requests to make")
	var shortConn = flag.Int("shortConnNum", 0, "numbers of concurrent short connection workers to run")
	var longConn = flag.Int("longConnNum", 1, "numbers of concurrent long connection workers to run")
	var qps = flag.Int("limit", 10000, "rate limit")
	var size = flag.Int("size", 16*10, "send buf size")
	var addr = flag.String("addr", "127.0.0.1:8000", "<IP:Port>")
