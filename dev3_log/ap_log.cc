...
void apLogServer::CreateLoopSqdbClTimer() {
	...
	sqdbCl* cl = pool->GetDbCl("dev3_log.dev3_log_dev");
	if (!cl) {	
...
void apLogServer::GetQosSwitchTimer() {
	...
	sqdbCl* cl = pool->GetDbCl("dev3_config.config");
	if (!cl) {
	...
	cl = pool->GetDbCl("dev3_config.log_qos_vip");
	if (!cl) {
		LOG_ERROR("GetDbCl dev3_config.log_qos_vip err");
		return ;
	}
...
void apLogServer::OnMessageProcess(MsgStringPtr &Message) {
	...
	// dev3_log_20180702.dev3_log_dev skip time  
	sqdbCl* cl = pool->GetDbCl("dev3_log.dev3_log_dev");
	if (!cl) {
		LOG_ERROR("get cl dev3_log.dev3_log_dev err");
		return ;
	}
	cl->insert(db_obj);		
