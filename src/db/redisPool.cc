...
int CacheConn::Init() {
	...
	if (!m_pContext || m_pContext->err) {
		if (m_pContext) {
			REDIS_ERROR("redisConnect failed: %s", m_pContext->errstr);
			redisFree(m_pContext);
			m_pContext = NULL;
		} else {
			REDIS_ERROR("redisConnect failed");
		}
	...
	if (reply && (reply->type == REDIS_REPLY_STATUS) && (strncmp(reply->str, "OK", 2) == 0)) {
		...
		REDIS_ERROR("select cache db failed");
		return 2;
	}
}
...
string CacheConn::get(string key) {
	...
	if (!reply) {
		REDIS_ERROR("redisCommand failed:%s", m_pContext->errstr);
		redisFree(m_pContext);
		m_pContext = NULL;
		return value;
	}
...
long CacheConn::incrBy(string key, long value) {
    ...
    if(!reply) {
        REDIS_ERROR("redis Command failed:%s", m_pContext->errstr);
        redisFree(m_pContext);
        m_pContext = NULL;
        return -1;
    }
...
int CachePool::Init() {
	...
	REDIS_DEBUG("cache pool: %s, list size: %lu", m_pool_name.c_str(), m_free_list.size());
	return 0;
}
...
CacheConn* CachePool::GetCacheConn() {
	...
	while (m_free_list.empty()) {
		if (m_cur_conn_cnt >= m_max_conn_cnt) {
			...
			if (ret) {
				REDIS_ERROR("Init CacheConn failed");
				delete pCacheConn;
				m_free_notify.Unlock();
				return NULL;
			} else {
				...
				m_cur_conn_cnt++;
				REDIS_DEBUG("new cache connection: %s, conn_cnt: %d", m_pool_name.c_str(), m_cur_conn_cnt);
			}
		}
	}
...
int CacheManager::Init() {
    std::string name = config_reader::ins()->GetString("redis", "name", "ap_log");
    std::string host = config_reader::ins()->GetString("redis", "host", "127.0.0.1");
    int  port = config_reader::ins()->GetNumber("redis", "port", 6379);
    //std::string user = config_reader::ins()->GetString("redis", "user", "sdbadmin");
    //std::string pwd = config_reader::ins()->GetString("redis", "pwd", "netcoresdbadmin");
