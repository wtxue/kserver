...
public:
    ...
    bool Listen(int p) {
        ...
        if (this->fd_ < 0) {
            LOG_ERROR("listen error port:%d",p);
            return false;
        }
...
void Server::RecvingLoop(RecvThread* thread) {
    LOG_INFO("UDPServer is running at 0.0.0.0:%d", thread->port());
    ...
    while (true) {
        ...
        if (readn >= 0) {
            LOG_TRACE("fd:%d port:%d recv_len:%d from %s",thread->fd(),thread->port(),readn,sock::ToIPPort(recv_msg->remote_addr()).c_str());
            ...
            LOG_ERROR("errno:%d %s",eno,strerror(eno));
        }
    }

    LOG_INFO("fd:%d port:%d UDP server existed.",thread->fd(),thread->port());
