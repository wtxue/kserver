...
TCPClient::TCPClient(EventLoop* l, const std::string& raddr, const std::string& n)
    ...
    DLOG_TRACE("create remote:%s",raddr.c_str());
}

TCPClient::~TCPClient() {
    DLOG_TRACE("destroy TCPClient");
...
void TCPClient::Connect() {
    LOG_INFO("remote_addr=%s",remote_addr().c_str());
    auto f = [this]() {
...
void TCPClient::Disconnect() {
    DLOG_TRACE("Disconnect");
    loop_->RunInLoop(std::bind(&TCPClient::DisconnectInLoop, this));
}

void TCPClient::DisconnectInLoop() {
    LOG_WARN("TCPClient::DisconnectInLoop  remote_addr=%s",remote_addr_.c_str());
    assert(loop_->IsInLoopThread());
    auto_reconnect_.store(false);

    if (conn_) {
        DLOG_TRACE("Close the TCPConn:%s status=%s", remote_addr_.c_str() , conn_->StatusToString().c_str());
    ...
    if (connector_->IsConnected() || connector_->IsDisconnected()) {
        DLOG_TRACE("Nothing to do with connector_, Connector::status=%d",connector_->status());
    } else {
...
void TCPClient::Reconnect() {
    DLOG_TRACE("Try to reconnect to %s in %fs again",remote_addr_.c_str(),reconnect_interval_.Seconds());
    Connect();
}
...
void TCPClient::OnConnection(net_socket_t sockfd, const std::string& laddr) {
    if (sockfd < 0) {
        //DLOG_TRACE("Failed to connect to %s errno=%d %s",remote_addr_.c_str(),errno,strerror(errno));
    ...
    DLOG_TRACE("Successfully connected to %s",remote_addr_.c_str(),name_.c_str());
    assert(loop_->IsInLoopThread());
