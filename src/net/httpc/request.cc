...
#include "net/httpc/url_parser.h"
...
Request::Request(EventLoop* loop, const std::string& http_url, const std::string& body, Duration timeout)
    ...
    LOG_INFO("LIBEVENT_VERSION_NUMBER:0x%08x",LIBEVENT_VERSION_NUMBER);
    ...
    LOG_DEBUG("uri_:%s",uri_.c_str());

    host_ = evhttp_uri_get_host(evuri);
    port_ = evhttp_uri_get_port(evuri);    
    LOG_DEBUG("host_:%s port_:%d",host_.c_str(),port_);
    ...
    LOG_DEBUG("enable_ssl:%d",enable_ssl);
...
failed:
    ...
    if (retried_ < retry_number_) {
		LOG_WARN("this:%p http request failed:%s retried:%d max retry_time:%d. Try again.", this, errmsg.c_str(), retried_, retry_number_);
        Retry();
        return;
    }
...
void Request::HandleResponse(struct evhttp_request* r) {
    ...
    DLOG_TRACE("HandleResponse");
    if (r) {
        int response_code = r->response_code;
		DLOG_TRACE("response_code:%d",response_code);
        bool needs_retry = response_code >= 500 && response_code < 600;
        if (!needs_retry || retried_ >= retry_number_) {
            LOG_WARN("this:%p response_code:%d retried:%d max retry_time:%d", this, r->response_code, retried_, retry_number_);
    ...
    // Retry
    if (retried_ < retry_number_) {
		LOG_WARN("this:%p response_code:%d retried:%d max retry_time:%d. Try again", this, r->response_code, retried_, retry_number_);
        Retry();
        return;
    }
    ...
    if (!r) {
        ...
        while ((oslerr = bufferevent_get_openssl_error(conn_->bufferevent()))) {
            ...
            LOG_ERROR("Openssl error: %s",buffer);
            printed_some_error = true;
        }
        if (!printed_some_error) {
            LOG_ERROR("socket error(%d): %s", errcode, evutil_socket_error_to_string(errcode).c_str());
        }
    }
#endif
