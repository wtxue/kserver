...
void DNSResolver::SyncDNSResolve() {
    ...
    if (err != 0) {
        LOG_ERROR("this=%p getaddrinfo failed. err=%d %s",this,err,gai_strerror(err));
    } else {
        for (struct addrinfo* rp = answer; rp != nullptr; rp = rp->ai_next) {
            ...
            DLOG_TRACE("host=%s resolved a ip=%s",host_.c_str(),inet_ntoa(a->sin_addr));
        }
    }
...
void DNSResolver::OnCanceled() {
	DLOG_TRACE("OnCanceled");
#if LIBEVENT_VERSION_NUMBER >= 0x02001500
	DLOG_TRACE("OnCanceled enter");
    evdns_getaddrinfo_cancel(dns_req_);
    dns_req_ = nullptr;
#endif
}
...
void DNSResolver::OnResolved(int errcode, struct addrinfo* addr) {
    if (errcode != 0) {
        if (errcode != EVUTIL_EAI_CANCEL) {
            ClearTimer();
            LOG_ERROR("DNS resolve failed, error code:%d error msg:%s",errcode,evutil_gai_strerror(errcode));
        } else {
            DLOG_WARN("DNS resolve cancel, may be timeout");
        }

        DLOG_WARN("delete DNS base. errcode=%d %s",errcode,strerror(errcode).c_str());
    ...
    if (addr == nullptr) {
        LOG_ERROR("this=%p dns resolve error, addr can not be nullptr",this);
    ...
    if (addr->ai_canonname) {
        DLOG_TRACE("resolve canon name: %s",addr->ai_canonname);
    }
...
void DNSResolver::OnResolved(int errcode, struct addrinfo* addr, void* arg) {
    ...
    //LOG_TRACE("this->use_count=%u",pp->use_count());
