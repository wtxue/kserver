...
struct utc_timer {
    ...
    uint64_t get_curr_time(long* p_msec = NULL)     {
        struct timeval tv;
        //get current ts
        gettimeofday(&tv, NULL);
        if (p_msec)
...
public:
    ...
    void init_path(const char* log_dir, const char* prog_name, bool is_console, int level);
...
#define LOG_INIT(log_dir, prog_name,is_console,level) \
    do \
    { \
        ring_log::ins()->init_path(log_dir, prog_name,is_console,level); \
        if (!is_console) { \
        	pthread_t tid; \
...
#define _LOG_TRACE(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= TRACE) \
        { \
            ring_log::ins()->try_append("[TRAE]", "[%s-%d-%s]: " fmt "\n", \
...
#define _LOG_DEBUG(fmt, args...) \
    ...
    { \
        ...
        { \
            ring_log::ins()->try_append("[DBUG]", "[%s-%d-%s]: " fmt "\n", \
                      __FILE__, __LINE__, __FUNCTION__, ##args); \
        } \
    } while (0)

#define _LOG_INFO(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= INFO) \
        { \
            ring_log::ins()->try_append("[INFO]", "[%s-%d-%s]: " fmt "\n", \
                      __FILE__, __LINE__, __FUNCTION__, ##args); \
        } \
    } while (0)

#define _LOG_NORMAL(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= INFO) \
        { \
...
#define _LOG_WARN(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= WARN) \
        { \
            ring_log::ins()->try_append("[WARN]", "[%s-%d-%s]: " fmt "\n", \
                    __FILE__, __LINE__, __FUNCTION__, ##args); \
        } \
    } while (0)

#define _LOG_ERROR(fmt, args...) \
    do \
    { \
        if (ring_log::ins()->get_level() >= ERROR) \
        { \
            ring_log::ins()->try_append("[ERRO]", "[%s-%d-%s]: " fmt "\n", \
                   __FILE__, __LINE__, __FUNCTION__, ##args); \
        } \
    } while (0)

#define _LOG_FATAL(fmt, args...) \
    do \
    { \
        ring_log::ins()->try_append("[FATL]", "[%s-%d-%s]: " fmt "\n", \
                   __FILE__, __LINE__, __FUNCTION__, ##args); \
    } while (0)
...
#define _LOG_HEAD(str, fmt, args...)     \
	do {	  \
		if (ring_log::ins()->get_level() >= ERROR) {\
			ring_log::ins()->try_append(str, "[%s-%d-%s]: " fmt "\n", \
				__FILE__, __LINE__, __FUNCTION__, ##args);	  \
		}		   \
	} while (0)
